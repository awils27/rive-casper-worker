...
import { generateTemplate } from './api.mjs';
...
async function onFileSelect(){
  ...
  // Compute the EXACT HTML filename once (preserve case where possible)
...
els.downloadBtn.addEventListener('click', async () => {
  ...
  const templateKey = document.querySelector('input[name="runtime"]:checked')?.value || 'caspar-basic';
  const payload = {
    template: templateKey,
    schema,
    filename: htmlFilename, // exact name the server should return
    options: {
      casparTriggers: {
  ...
  try {
    ...
    a.download = res.headers.get('content-disposition')?.match(/filename="([^"]+)"/)?.[1] || htmlFilename;
    a.click();
